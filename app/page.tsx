      ...
      <main className="max-w-7xl mx-auto px-8 py-10">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setCurrentIdx(Math.max(0, currentIdx - 3))} className="border p-2 rounded-full hover:bg-gray-50">←</button>
            <button onClick={() => setCurrentIdx(Math.min(7, currentIdx + 3))} className="border p-2 rounded-full hover:bg-gray-50">→</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          {top10Guides.slice(currentIdx, currentIdx + 3).map((g) => (
            <div key={g.id} className="border p-4 rounded-xl shadow-sm hover:shadow-md transition">
              ...
              <p className="text-sm font-semibold text-gray-900">★ {g.rating}</p>
            </div>
          ))}
        </div>
      </main>
